...
/**
 * The submitted-applications grid (formerly the standalone Tracker page), now the
 * "Submitted" segment of the Jobs screen. Read-only credibility view; rows link
 * through to the job so it's no longer a dead-end.
 */
    ...
    <div className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-lg border border-coffee-300 bg-white">
      {isError ? (
        <div className="flex flex-1 items-center justify-center p-8">
          <ErrorState
            description="Couldn't load your applications. The backend may be offline."
            retry={() => refetch()}
          />
        </div>
      ) : (
          ...
          <DataTable<TrackerApplication>
            ...
            emptyState={
              <EmptyState
                ...
                description="Applications appear here once you (or a VA) apply to a job."
              />
            }
          />
        </div>
