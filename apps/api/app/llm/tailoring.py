...
async def tailor(
    ...
    system = (
        ...
        "BULLET QUALITY: prefer PAR (Problem-Action-Result) or XYZ (accomplished X, "
        "measured by Y, doing Z) structure, opening with a strong, accurate action verb "
        "(led, built, engineered, delivered — not managed/worked/helped/responsible for) "
        "ONLY when it truthfully reflects the candidate's role. Never fabricate a metric "
        "to fit the format.\n"
        "SUMMARY: 'summary' must be a concise, recruiter-focused professional summary "
        "tailored to this role — never a generic 'Objective'.\n"
        "KEYWORDS: incorporate the JD's real required skills the candidate genuinely has, "
        "naturally and in context — never keyword-stuff or repeat a term unnaturally.\n"
