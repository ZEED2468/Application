...
def emit(event_name: str, payload: EventPayload) -> None:
    ...
    # Publishing is fire-and-forget: a broker hiccup must NOT fail the user action
    # that triggered it (the originating DB write is already committed). We disable
    # the result backend on publish and swallow transport errors, logging them so
    # they're visible. In a healthy deploy this always succeeds.
    try:
        ...
        # NB: `event` is structlog's reserved message key — use event_name=.
        log.info("event.emitted", event_name=event_name, user_id=str(validated.user_id))
    except Exception as exc:  # broker unreachable / transport error
        log.error("event.emit_failed", event_name=event_name, error=str(exc))
