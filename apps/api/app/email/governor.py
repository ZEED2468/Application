...
async def governed_relay(
    session: AsyncSession, *, user_id, sending_domain_id, send_fn
) -> tuple[SendResult, str | None]:
    """Send a one-off outbound (a VA-relayed reply) under the same caps as outreach.

    `send_fn(domain) -> message_id` does the provider call. Same single choke point,
    so VA relays can never bypass warm-up either.
    """
    now = datetime.now(timezone.utc)
    domain = (
        await session.execute(
            select(SendingDomain).where(SendingDomain.id == sending_domain_id).with_for_update()
        )
    ).scalar_one()

    if domain.is_paused:
        return SendResult.paused, None
    if caps.is_new_day(domain.daily_count_date, now.date()):
        domain.daily_sent_count = 0
        domain.daily_count_date = now.date()
    domain.warmup_stage = caps.stage_for_age(domain.warmup_started_at, now)
    cap = caps.daily_cap(domain.warmup_stage)
    weekly = await _weekly_count(session, user_id, now)
    if domain.daily_sent_count >= cap or weekly >= settings.weekly_cap_per_hunter:
        return SendResult.deferred, None

    message_id = await send_fn(domain)
    domain.daily_sent_count += 1
