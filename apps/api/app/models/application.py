...
from app.core.enums import ApplicationStatus, TrackerStatus
...
class Application(Base, TimestampMixin):
    ...
    # The VA-facing tracker status (dashboard dropdown). Distinct from the internal
    # lifecycle `status` above; both paths converge here so the tracker stays in sync.
    tracker_status: Mapped[TrackerStatus] = mapped_column(
        Enum(TrackerStatus, native_enum=False),
        default=TrackerStatus.applied,
        nullable=False,
    )
