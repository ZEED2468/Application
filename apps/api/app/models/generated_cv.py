...
from sqlalchemy import Enum, Float, ForeignKey, String, Text, Uuid, UniqueConstraint
...
class GeneratedCv(Base, TimestampMixin):
    ...
    # Which uploaded source CV the manual path tailored from (null on auto path).
    source_role_cv_id: Mapped[uuid.UUID | None] = mapped_column(
        Uuid, ForeignKey("role_cv.id", ondelete="SET NULL"), nullable=True
    )
    cv_json: Mapped[dict] = mapped_column(JsonB, default=dict)
    # Internal ATS-match score (0-100) + breakdown. NOT an employer-ATS guarantee.
