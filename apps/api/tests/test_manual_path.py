...
@pytest.mark.asyncio
async def test_create_job_from_jd_creates_and_dedupes(session):
    """The "Tailor" nav entry: a pasted JD becomes a manual job (no generation), and
    re-pasting the same JD reuses it — so both doors land in one workspace."""
    user, _ = await seed_hunter(session)
    job1 = await service.create_job_from_jd(
        session, user_id=user.id, jd_text=JD,
        role_title="Senior Backend Engineer", track="backend",
    )
    assert job1.source is JobSourceName.manual and job1.origin is Origin.manual
    assert job1.track is Track.backend
    assert job1.description == JD and job1.role_title == "Senior Backend Engineer"

    # Same JD reuses the job (dedupe on the JD), and NO CV is generated by this entry —
    # the workspace's Generate button runs the shared engine.
    job2 = await service.create_job_from_jd(session, user_id=user.id, jd_text=JD)
    assert job2.id == job1.id
    cv = (
        await session.execute(select(GeneratedCv).where(GeneratedCv.job_id == job1.id))
    ).scalar_one_or_none()
    assert cv is None
